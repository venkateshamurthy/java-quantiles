...
public class PerfTestUtils {
	 ...
	 * @param runGC
	 *            Call {@code System.gc()} between each timed block. When set to
	 ...
	 */
	public static StatisticalSummary[] time(int repeatChunk, int repeatStat,
			boolean runGC, Callable<Double>... methods) {
		final double[][][] times = timesAndResults(repeatChunk, repeatStat,
				runGC, methods);
	 ...
	 * @param methods
	 *            Codes being timed.
	 * @return for each of the given {@code methods} (first dimension), and each
	 ...
	 *         </ul>
	 */
	public static double[][][] timesAndResults(int repeatChunk, int repeatStat,
			boolean runGC, Callable<Double>... methods) {
		...
		try {
			...
			throw new MathIllegalStateException(
					LocalizedFormats.SIMPLE_MESSAGE, e.getMessage());
		}
	...
	public static StatisticalSummary[] timeAndReport(String title,
			int repeatChunk, int repeatStat, boolean runGC, RunTest... methods) {
		// Header format.
		...
		System.out.println(String.format(hFormat, title, repeatChunk,
				repeatStat));
		System.out.println(String.format(cFormat, "name", "time/call",
				"std error", "total time", "ratio", "difference"));
		final StatisticalSummary[] time = time(repeatChunk, repeatStat, runGC,
				methods);
		...
		for (int i = 0, max = time.length; i < max; i++) {
			...
			System.out.println(String.format(format, methods[i].getName(),
					s.getMean(), s.getStandardDeviation(), sum, sum / refSum,
					sum - refSum));
		}
	...
	public static StatisticalSummary[] timeAndReport(String title,
			RunTest... methods) {
		return timeAndReport(title, DEFAULT_REPEAT_CHUNK, DEFAULT_REPEAT_STAT,
				false, methods);
	}
	/**
	 * Run Long running tests and return an array of statistical summary
	 * @param longRunningTests
	 * @return StatisticalSummary[]
	 */
	public static StatisticalSummary[] runLongRunTests(
			LongRunTest... longRunningTests) {
		final int numMethods = longRunningTests.length;
		final List<StatisticalSummary> timesAndResults = new ArrayList<StatisticalSummary>(numMethods);
		final ExecutorService executorService = Executors
				.newFixedThreadPool(numMethods);
		final CompletionService<StatisticalSummary> service = new ExecutorCompletionService<StatisticalSummary>(
				executorService);
		try {
			List<Future<StatisticalSummary>> tasks = new ArrayList<Future<StatisticalSummary>>();
			for (int j = 0; j < numMethods; j++) {
				final Callable<StatisticalSummary> r = longRunningTests[j];
				tasks.add(service.submit(r));
			}

			try {
				for(Future<StatisticalSummary> s:tasks)
					timesAndResults.add(s.get());
				while (!tasks.isEmpty()) {
					Future<StatisticalSummary> future = service.take();
					tasks.remove(future);
				}
		...
		for(LongRunTest m:longRunningTests){
			sb.append(m);
		}
		System.out.println(sb.toString());
		return (StatisticalSummary[])timesAndResults.toArray(new StatisticalSummary[1]);
	}
	...
	/**
	 * Utility class for running a log running test.
	 */
	public static class LongRunTest implements Callable<StatisticalSummary> {
		private final String testName;
		private final int count;
		private final RunTest runTest;
		private volatile double lastValue;
		private final boolean shouldDelay;
		private final int calls,sleepMicros;
		private final SummaryStatistics timeSummary = new SummaryStatistics();
		public LongRunTest(String testName,RunTest runTest, int count){
			this(testName,runTest,count,0,0);
		}
		/**
		 * @param name
		 *            Test name.
		 */
		public LongRunTest(String testName,RunTest runTest, int count,int calls,int sleepMicros) {
			this.testName=testName;
			this.count = count;
			this.runTest = runTest;
			this.sleepMicros=sleepMicros;
			this.calls=calls;
			shouldDelay=sleepMicros>0&&calls>0;
		}
		...
		public StatisticalSummary call() throws Exception {
			...
			for (int i = 0; i < count; i++) {
				...
				lastValue=runTest.call();
				if(shouldDelay && i%calls==0)
					LockSupport.parkNanos(sleepMicros*1000);
				long t2 = System.nanoTime();
				timeSummary.addValue((t2 - t1)*NANO_TO_MILLI);
			}
			return timeSummary;
		}
		...
		public String toString(){
			return String.format(" %s=%.4f(%.4f) ",  testName,timeSummary.getSum(),lastValue);
		}
	}
}
